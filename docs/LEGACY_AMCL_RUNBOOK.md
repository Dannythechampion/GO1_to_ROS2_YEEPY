# 레거시 운영 문서 — AMCL fallback과 수동 실행 절차

> [!WARNING]
> 이 문서는 2026-09-22 이전 루트 README의 본문(AMCL fallback, 터미널별 수동 실행,
> 문제 해결)을 그대로 옮긴 **기록용 문서**입니다. 현재 권장 경로는
> [루트 README](../README.md)의 pose-graph localization(`go1_posegraph_navigation.launch.py`)입니다.
> 이 문서의 AMCL 경로는 `arm:=false` 진단에만 쓰며, 실제 저속 시험의 유일한 진입점은
> `./migration/jetson_field_deploy.sh armed GO1_ARMED_AND_ESTOP_READY`입니다.
> 아래 내용 중 브랜치 이름, 경로, 검증 수치는 작성 당시 기준입니다.
...
> 기본값은 항상 `arm:=false`입니다. armed 실행은 [루트 README](../README.md)의 Jetson runner와
...
이 절의 `go1_existing_map.launch.py`는 레거시 AMCL fallback입니다. 신규 운용은 [루트 README](../README.md)의
pose-graph 절차를 따릅니다. 레거시 fallback을 진단 목적으로 사용할 때에도
...
허용되며, 실제 저속 시험은 [루트 README](../README.md)의 `jetson_field_deploy.sh`만 사용합니다.
...
현재 `DRY-RUN`은 정상입니다. 직접 launch 인자를 바꿔 armed로 전환하지 말고, [루트 README](../README.md)의
fail-closed Jetson runner와 현장 체크리스트만 사용합니다.
